...
def _system_prompt(user: User) -> str:
    return (
        ...
        "ready to copy: a header line with the date and the engineer's name, then ALL "
        "THREE sections with tight bullets ('- item — state — next step'). Never omit "
        "a section: if the user skipped it or answered 'none'/'no'/'-'/empty, still "
        "print the section header with 'none' as its content (e.g. 'Ongoing tasks: "
        "none'). The handover post is ALWAYS written in English, even if the "
